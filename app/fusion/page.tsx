...
export default function FusionPage() {
  ...
  const bgs = useUploadItems();
  const poses = useUploadItems();
  ...
  const expectedResultCount = poses.items.length * safeCount;
  ...
  const ensureAssetsUploaded = async () => {
    ...
    const uploadedFaces = await uploadItemsToStorage(faces.items, "faces", fusionSessionId!);
    faces.setItems(uploadedFaces);

    const uploadedOutfits = await uploadItemsToStorage(outfits.items, "outfits", fusionSessionId!);
    outfits.setItems(uploadedOutfits);

    const uploadedBgs = await uploadItemsToStorage(bgs.items, "bgs", fusionSessionId!);
    bgs.setItems(uploadedBgs);

    const uploadedPoses = await uploadItemsToStorage(poses.items, "poses", fusionSessionId!);
    poses.setItems(uploadedPoses);
  ...
  const resetUploadedStorageState = () => {
    ...
    bgs.setItems((prev) => prev.map(resetStorage));
    poses.setItems((prev) => prev.map(resetStorage));
  };
        ...
        <div className="grid grid-cols-1 xl:grid-cols-2 gap-6 mb-6">
          <div className="space-y-4">
            ...
            <UploadSection
              ...
              items={bgs.items}
              onAddFiles={bgs.appendFiles}
              onRemoveItem={bgs.removeItem}
              onClearAll={bgs.clearAll}
            />
          </div>
          ...
          <UploadSection
            ...
            items={poses.items}
            onAddFiles={poses.appendFiles}
            onRemoveItem={poses.removeItem}
            onClearAll={poses.clearAll}
          />
        </div>
        ...
        <div className="border rounded-2xl p-6 bg-white space-y-5">
          ...
          <div className="border rounded-xl p-4 bg-[#fafaf8]">
            ...
            <div className="text-sm text-gray-700 space-y-1">
              ...
              <div>BG: {bgs.items.length}장</div>
              <div>POSE: {poses.items.length}장</div>
